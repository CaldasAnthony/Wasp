...
def atmosphere_plot(I_tot,h,param,factor,r_step,theta_number,wl,bande_sample,name,Rp,R_s,extra,where,Convention,Kcorr,Middle,Cache) :

    theta_step = 2*np.pi/np.float(theta_number)
    R = Rp/np.float(factor)
    dim = int((R+1.10*h)/param)
    ...
    print 'Exoplanet radius : %i m' % (Rp)
    print 'Reduction factor for the exoplanet radius : %i' % (factor)
    print 'Map resolution : %ix%i' % (dim, dim)
    print 'Map step : %i m' % (param)
    print 'Selected wavelength : %.3f'%(wl_sample_micron[i_bande])

    I_png = np.ones((2*dim+1,2*dim+1))
    theta_png = np.ones((2*dim+1,2*dim+1))
        ...
        for x in range(-dim,dim+1) :
            ...
            if rho <= R + h :
                ...
                if rho >= R :
                    ...
                    if Convention == 'equatorial' :
                        theta = np.arctan2(y * param, x * param)
                    if Convention == 'polar' :
                        theta = np.arctan2(-x * param, y * param)
                    if theta < 0 :
                        theta = 2*np.pi + theta
                ...
                theta_png[y+dim,x+dim] = theta_line
    ...
    if Cache == True :
        ...
        I_png[wh[0],wh[1]] = 'nan'
    Z = I_png
    R_eff_bar,R_e,ratio_bar,ratR_bar,bande_bar,flux_bar,flux = atmospectre(I_tot,bande_sample,R_s,Rp,r_step*factor,extra,False,Kcorr,Middle)
    ...
    plt.figure(figsize=(12, 9))
    plt.imshow(theta_png, extent=[-dim * param / 1000., dim * param / 1000., -dim * param / 1000., dim * param / 1000.],
               origin='lower')
    plt.colorbar()
    plt.show()
...
class JWST :
    def __init__(self):
        ...
        self.bande = np.array([[0.4,1.6],[1.58,3.],[2.98,5.],[4.98,10.5],[10.48,16.],[15.98,27.5]])
